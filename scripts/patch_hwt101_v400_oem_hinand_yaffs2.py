...
if old_cb not in h: raise SystemExit('V3.68 probe callback sequence not found')
h = h.replace(old_cb,new_cb,1)
# V3.50 already sets the exact pre-ident FIX10 option value 0x501.
if 'h->chip.options|=(NAND_NO_AUTOINCR|NAND_NO_READRDY|NAND_BROKEN_XD);' not in h:
    raise SystemExit('V3.50 OEM chip options not present')
...
if n!=1: raise SystemExit('V3.67 Micron 0x88 ID entry not found')
ids.write_text(s)
...
if old not in s: raise SystemExit('fixed-page oobsize anchor not found')
s=s.replace(old,new,1); base.write_text(s)

s=cmd.read_text()
changes=[
('#define SIZE_REMAINING UINT_MAX','#define SIZE_REMAINING (~0ULL)'),
('#define OFFSET_CONTINUOUS UINT_MAX','#define OFFSET_CONTINUOUS (~0ULL)'),
('\tunsigned long size;\n\tunsigned long offset = OFFSET_CONTINUOUS;','\tunsigned long long size;\n\tunsigned long long offset = OFFSET_CONTINUOUS;'),
('partition size too small (%lx)','partition size too small (%llx)'),
('\tunsigned long offset;\n\tint i;\n\tstruct cmdline_mtd_partition *part;','\tunsigned long long offset;\n\tint i;\n\tstruct cmdline_mtd_partition *part;')]
for a,b in changes:
    if a not in s: raise SystemExit('cmdlinepart anchor missing: '+repr(a))
    s=s.replace(a,b,1)
cmd.write_text(s)

h=hp.read_text(); sids=ids.read_text(); sb=base.read_text(); sc=cmd.read_text()
checks=[
('OEM diag marker','HWT101_V400_HINAND_DIAG' in h),('OEM eccbytes','.eccbytes = 48' in h),
('OEM eccpos start','.eccpos = { 32, 33, 34' in h),('OEM eccpos end','77, 78, 79 }' in h),
('OEM oobfree','.offset = 2, .length = 30' in h),('ECC none','h->chip.ecc.mode=NAND_ECC_NONE' in h),
...
('OEM OOB core','mtd->oobsize = 448;' in sb),('64-bit partition size','unsigned long long size;' in sc),
('64-bit partition offset','unsigned long long offset = OFFSET_CONTINUOUS;' in sc),('64-bit parser offset','unsigned long long offset;' in sc)]
failed=[name for name,ok in checks if not ok]
if failed: raise SystemExit('V400 gate failed: '+', '.join(failed))
