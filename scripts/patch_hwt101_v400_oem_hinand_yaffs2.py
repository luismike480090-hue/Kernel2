...
# ------------------------------------------------------------------
# 1) HiNAND: replace the V3.68 guessed geometry/OOB block with the
#    exact nand_ecclayout recovered from the functional FIX10 kernel.
# ------------------------------------------------------------------
h = hp.read_text()
...
# Remove the V3.68 board init_size callback from probe and install OEM layout.
...
# FIX10 sets pre-ident options 0x501 (NO_AUTOINCR | NO_READRDY | 0x400).
# nand_get_flash_type later replaces chip-specific bits with the ID-table value.
old_opt = 'h->chip.options|=NAND_NO_AUTOINCR;'
new_opt = 'h->chip.options|=(NAND_NO_AUTOINCR|NAND_NO_READRDY|NAND_BROKEN_XD);'
if old_opt not in h:
    raise SystemExit('chip options anchor not found')
h = h.replace(old_opt, new_opt, 1)

# Correct diagnostic strings left by V3.68; init_size is intentionally unused.
h = h.replace('OOBFREE_LENGTH=446', 'OOBFREE_LENGTH=30')
h = h.replace('HWT101_V369_HINAND_DIAG', 'HWT101_V400_HINAND_DIAG')
hp.write_text(h)

# ------------------------------------------------------------------
# 2) NAND ID: reproduce the exact FIX10 table entry recovered from
#    OEM raw kernel table at 0xc080ab8c.
# ------------------------------------------------------------------
s = ids.read_text()
...
# ------------------------------------------------------------------
# 3) NAND core: Huawei fixed-page path reports OOB=448 for this exact
#    Micron 0x2c:0x88 device. Do this narrowly instead of globally.
# ------------------------------------------------------------------
s = base.read_text()
...
# ------------------------------------------------------------------
# 4) cmdlinepart: fix ARM32 truncation of explicit 5174M userdata.
#    memparse() already returns unsigned long long; preserve it end-to-end.
# ------------------------------------------------------------------
s = cmd.read_text()
changes = [
...
# ------------------------------------------------------------------
# Hard gates: fail build if any known-dangerous V3.68/V3.70 artifact remains.
# ------------------------------------------------------------------
...
print('BASE=EXACT_V369_WITH_V349_OEM_CLOCK_IRQ_AND_V366_MAP')
