...
# V4.01 diagnostic safety layer. Block the only two reconstructed controller
# paths that can physically program/erase NAND. READID/STATUS/RESET/READSTART
# remain byte-for-byte V4.00/OEM-parity paths.
old_prog='''static void hwt_pageprog(struct hwt_hinand *h) { hwt_clear_int(h); nfc_write(h,0x200,NFC_INTEN); nfc_write(h,0x3f,NFC_DMA_CFG); nfc_write(h,0x4c7,NFC_CON); nfc_write(h,(u32)h->dma_handle,NFC_DMA_ADDR); nfc_write(h,(u32)h->dma_handle+0x2000,NFC_DMA_ADDR2); nfc_write(h,0x01c00000,NFC_DMA_LEN2); hwt_set_addr(h); nfc_write(h,0x1080,NFC_CMD); nfc_write(h,0x23,NFC_DMA_CTRL); hwt_wait_int(h); h->addr_cycle=0; }'''
...
if old_prog not in s:
    raise SystemExit('V4.00 PAGEPROG implementation anchor not found')
s=s.replace(old_prog,new_prog,1)

old_erase='''case NAND_CMD_ERASE2: hwt_clear_int(h); nfc_write(h,1,NFC_INTEN); nfc_write(h,0xc7,NFC_CON); nfc_write(h,3,NFC_DMA_CFG); nfc_write(h,h->addr[0],NFC_ADDRL); nfc_write(h,h->addr_cycle>4?h->addr[1]:0,NFC_ADDRH); nfc_write(h,0xd060,NFC_CMD); nfc_write(h,(h->addr_cycle<<9)|0x6d,NFC_OP); hwt_wait_int(h); break;'''
new_erase='''case NAND_CMD_ERASE2:\n        printk(KERN_ERR "HWT101_V401_RO_GUARD: BLOCK ERASE2\\n");\n        h->addr_cycle=0;\n        break;'''
if old_erase not in s:
    raise SystemExit('V4.00 ERASE2 implementation anchor not found')
s=s.replace(old_erase,new_erase,1)

# Upper-layer protection too: after successful nand_scan, expose the master MTD
# and all partitions read-only. This happens before parse/register.
...
if old_scan not in s:
    raise SystemExit('V4.00 post-nand_scan anchor not found')
...
print('MTD_WRITEABLE=CLEARED_AFTER_NAND_SCAN')
print('READSTART=PRESERVED')
print('OEM_NFC_ECC_0x4c7=PRESERVED')
