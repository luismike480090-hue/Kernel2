...
static struct i2c_board_info hwt101_i2c_bus1_devs[] = {
    { .type = "sn65dsi83", .addr = 0x2d, .flags = true, .platform_data = &hwt101_sn65_pdata, },
};
...
if not src.exists() or not tab.exists(): raise SystemExit('missing recovered SN65 source/table')
dst=K/'drivers/video/k3'
(dst/'sn65dsi83_hwt101.c').write_text(src.read_text())
(dst/'sn65dsi83_oem_table.h').write_text(tab.read_text())
mk=dst/'Makefile'; ms=mk.read_text(); line='obj-y += sn65dsi83_hwt101.o\n'
if line not in ms: mk.write_text(ms+'\n# HWT101 MS1211 display bridge recovered from FIX10\n'+line)

helper=Path('scripts/add_hwt101_boot_abi.py')
if not helper.exists(): raise SystemExit('missing scripts/add_hwt101_boot_abi.py')
...
if not hinand.exists(): raise SystemExit('missing scripts/add_hwt101_hinand_v340.py')
exec(compile(hinand.read_text(), str(hinand), 'exec'), {'__name__':'__main__','__file__':str(hinand)})

cfg=K/'.config'; cs=cfg.read_text()
def set_y(name):
    global cs
    cs=re.sub(r'^CONFIG_'+re.escape(name)+r'=.*\n','',cs,flags=re.M)
    cs=re.sub(r'^# CONFIG_'+re.escape(name)+r' is not set\n','',cs,flags=re.M)
    cs+='CONFIG_'+name+'=y\n'
for opt in ('MTD','MTD_BLOCK','MTD_PARTITIONS','MTD_CMDLINE_PARTS','MTD_NAND','MTD_NAND_IDS','YAFFS_FS','YAFFS_YAFFS2'): set_y(opt)
cfg.write_text(cs)

patched=board.read_text()
platform_block=patched[patched.index('static struct platform_device *k3v2oem1_public_dev'):patched.index('static void k3v2_i2c_devices_init')]
for bad in ('btbcm_device','bcm_bluesleep_device','modem_switch_device','hisik3_battery_monitor'):
    if bad in platform_block: raise SystemExit('forbidden early device remains: '+bad)
for good in ('k3_lcd_device','usb_switch_device','hisik3_watchdog_device'):
    if good not in platform_block: raise SystemExit('required platform item missing: '+good)
for good in ('sn65dsi83','0x2d','hwt101_i2c_bus1_devs'):
    if good not in patched: raise SystemExit('required HWT101 I2C item missing: '+good)
if 'REGULATOR_SUPPLY("lcd-vcc", "sn65dsi83")' not in reg.read_text(): raise SystemExit('OEM SN65 LDO17 supply missing')
if not (K/'drivers/mtd/nand/hinand_hwt101.c').exists(): raise SystemExit('HWT101 hinand source missing')
